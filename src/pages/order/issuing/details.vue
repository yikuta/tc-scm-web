<template>
  <div class="page-container">
    <el-card class="status-content">
      <div class="title">
        <span>当前状态：</span>
        <span v-show="data.orderStatus == 10">待处理</span>
        ...
        <span v-show="data.orderStatus == 55">取消</span>
      </div>
    </el-card>
...
<script>
import { omsOrderInfo } from '@/api/order'
...
export default {
  ...
  mounted() {
    this._omsOrderInfo(this.$route.query.id)
  },
  methods: {
    onReresh() {
      this._omsOrderInfo(this.$route.query.id)
    ...
    _omsOrderInfo(id) {
      return omsOrderInfo(id).then((res) => {
        if (res.code === 200) {
