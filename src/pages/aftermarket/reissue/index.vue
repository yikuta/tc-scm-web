  ...
  <div class="page-container">
    <el-card class="search-content">
      <el-form ref="search" :model="form" label-width="100px" size="mini">
        <el-form-item label="补发单号" prop="code.value">
        ...
        <el-form-item label="补发状态" prop="status.value">
          <el-select v-model="form.status.value" clearable>
            <el-option label="全部" value=""></el-option>
            <el-option label="待补发" :value="1"></el-option>
    ...
    <el-table :data="tableData" size="mini">
			...
			<el-table-column label="补发单" prop="afterSaleOrderNo"></el-table-column>
			...
			<el-table-column label="补发状态" prop="status.describe">
        <template v-slot:default="{ row }">
          <span v-if="row.transStatus == 1">已补发</span>
          <span v-else>待补发</span>
        </template>
      </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      showDetail(row) {
        this.$router.push({ path: '/aftermarket/reissue/details', query: { id: row.id } })
      },
