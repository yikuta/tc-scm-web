...
<script>
  ...
  export default {
    ...
    computed: {
      visitedViews() {
        return this.$store.state.global.visitedViews
      }
    },
    watch: {
      $route(to, from) {
    ...
    methods: {
      ...
      closeSelectedTag(view) {
        this.$store.dispatch('deleteVisitedViews', view).then((views) => {
...
<style type="text/scss" lang="scss" scoped>
  ...
  .tags-view-container {
    .tags-view-wrapper {
      background: #fff;
      height: 45px;
      box-shadow: 0 1px 3px 0 rgba(0, 0, 0, .08), 0 0 3px 0 rgba(0, 0, 0, .02);
